...
// CONFIG
const C = {
  PRIVATE_KEY: process.env.PRIVATE_KEY as Address,
  ...
  TP1: parseFloat(process.env.TAKE_PROFIT_1 || "1.5"),
  TP2: parseFloat(process.env.TAKE_PROFIT_2 || "2.0"),
...
async function feed(text: string, type: FT, ta?: string, ts?: string) {
  ...
  try {
    await db.from("feed").insert({ text, type, token_address: ta || null, token_symbol: ts || null });
  } catch {}
}
...
async function isSafe(addr: string): Promise<{ ok: boolean; why?: string }> {
  try {
    ...
    const d = await r.json();
    const i = d.result?.[addr.toLowerCase()];
...
async function fmInfo(token: Address) {
  try {
    ...
    const [,,, lastPrice,,,,offers, maxOffers,,, liquidityAdded] = r;
    const progress = maxOffers > 0n ? Number(offers * 100n / maxOffers) : 0;
    return { lastPrice, progress: 100 - progress, liquidityAdded };
  } catch {
    return null;
  }
}
...
async function buyFM(token: Address, bnb: string): Promise<Hash | null> {
  try {
    await initNonce();
    const f = parseEther(bnb);
    return await wall.sendTransaction({
      to: FM_LAUNCHPAD,
      data: encodeFunctionData({ abi: FM_BUY_ABI, functionName: "buyTokenAMAP", args: [0n, token, f, 0n] }),
      value: f, gas: 300_000n, nonce: useNonce(),
    });
  } catch (e: any) {
    console.error("buy fail:", e.shortMessage || e.message);
    await resetNonce();
    return null;
  }
}
...
async function sellFM(token: Address, amount: bigint): Promise<Hash | null> {
  try {
    ...
    await wall.sendTransaction({
      to: token,
      data: encodeFunctionData({ abi: ERC20_ABI, functionName: "approve", args: [FM_LAUNCHPAD, amount] }),
      gas: 100_000n, nonce: useNonce(),
    });
    return await wall.sendTransaction({
      to: FM_LAUNCHPAD,
      data: encodeFunctionData({ abi: FM_SELL_ABI, functionName: "sellToken", args: [token, amount] }),
      gas: 300_000n, nonce: useNonce(),
    });
  } catch (e: any) {
    console.error("sell fail:", e.shortMessage || e.message);
    await resetNonce();
    return null;
  }
}
...
async function buyFlap(token: Address, bnb: string): Promise<Hash | null> {
  try {
    ...
    return await wall.sendTransaction({
      ...
      value: f, gas: 350_000n, nonce: useNonce(),
    });
  } catch (e: any) {
    console.error("buy fail:", e.shortMessage || e.message);
    await resetNonce();
  ...
  try {
    ...
    return await wall.sendTransaction({
      ...
      data: encodeFunctionData({
        ...
        args: [{ inputToken: token, outputToken: ZERO, inputAmount: amount, minOutputAmount: 0n, permitData: "0x" as Address }],
      }),
      gas: 350_000n, nonce: useNonce(),
    });
  } catch (e: any) {
    console.error("sell fail:", e.shortMessage || e.message);
    await resetNonce();
...
async function evaluate(token: Address, platform: "four_meme" | "flap_sh", name?: string, symbol?: string) {
  ...
  if (!name || !symbol) {
    try {
      const results = await Promise.all([
        pub.readContract({ address: token, abi: ERC20_ABI, functionName: "name" }),
        pub.readContract({ address: token, abi: ERC20_ABI, functionName: "symbol" }),
      ]);
      name = results[0];
      symbol = results[1];
    } catch {
  ...
  if (!sec.ok) {
    await feed(symbol + " rejected — " + sec.why, "reject", token, symbol);
    await think("Token " + symbol + " failed: " + sec.why + ". React.");
    return;
  }
  await feed("security passed", "system", token, symbol);

  if (platform === "four_meme") {
    const info = await fmInfo(token);
    if (info) {
      if (info.liquidityAdded) {
        await feed(symbol + " already on DEX", "reject", token, symbol);
        return;
      }
      if (info.progress > 80) {
        await feed(symbol + " bonding " + info.progress + "% — too late", "reject", token, symbol);
        return;
      }
      await feed("bonding " + info.progress + "%", "system", token, symbol);
    }
  }
  ...
  if (!tx) {
    await feed(symbol + " buy failed", "reject", token, symbol);
    return;
  }
  ...
  try {
    ...
    if (receipt.status === "reverted") {
      await feed(symbol + " reverted", "reject", token, symbol);
      await logTrade(token, name!, symbol!, platform, "buy", C.BUY_AMOUNT, "0", tx, "failed");
      return;
    }
...
function startFM() {
  ...
  pub.watchContractEvent({
    address: FM_LAUNCHPAD,
    abi: FM_PURCHASE_EVENT,
    eventName: "TokenPurchase",
    onLogs: (logs) => {
      for (const l of logs) {
        const t = l.args.token;
        const s = l.args.sender;
        if (!t || s?.toLowerCase() === account.address.toLowerCase()) continue;
        if (!seen.has(t.toLowerCase())) evaluate(t, "four_meme");
      }
    },
...
function startFlap() {
    ...
    onLogs: (logs) => {
      for (const l of logs) {
        ...
        if (t && !seen.has(t.toLowerCase())) {
          evaluate(t as Address, "flap_sh", l.args.name, l.args.symbol);
        }
      }
    },
  ...
  pub.watchContractEvent({
    ...
    onLogs: (logs) => {
      for (const l of logs) {
        const t = l.args.token;
        if (t && !seen.has(t.toLowerCase())) evaluate(t as Address, "flap_sh");
      }
    },
    onError: () => {},
  });
...
async function monitor() {
  for (const [key, pos] of positions) {
    try {
      const age = (Date.now() - pos.time.getTime()) / 60_000;
      let mult = 1;

      if (pos.platform === "four_meme") {
        const info = await fmInfo(pos.addr);
        if (info && pos.entryPrice > 0n) {
          mult = Number(info.lastPrice) / Number(pos.entryPrice);
        } else {
          const q = await fmSellQuote(pos.addr, pos.tokens);
          if (q) {
            const cw = parseEther(pos.cost);
            if (cw > 0n) mult = Number(q.funds) / Number(cw);
          }
        }
      } else {
        try {
          const r = await pub.simulateContract({
            ...
            args: [{ inputToken: pos.addr, outputToken: ZERO, inputAmount: pos.tokens }],
          });
          const cw = parseEther(pos.cost);
          if (cw > 0n) mult = Number(r.result) / Number(cw);
        } catch {}
      }
      ...
      // TP1
      if (mult >= C.TP1 && !pos.halfSold) {
        await feed("TP1 — selling 50% " + pos.symbol, "action", pos.addr, pos.symbol);
        const h = pos.tokens / 2n;
        const tx = pos.platform === "four_meme" ? await sellFM(pos.addr, h) : await sellFlap(pos.addr, h);
        if (tx) {
          pos.halfSold = true;
          pos.tokens -= h;
          await feed("secured", "confirm", pos.addr, pos.symbol);
          await logTrade(pos.addr, pos.name, pos.symbol, pos.platform, "sell", "partial", h.toString(), tx, "confirmed");
          await think("Sold half " + pos.symbol + " at " + mult.toFixed(1) + "x. React.");
        }
      }

      // TP2
      if (mult >= C.TP2 && pos.halfSold) {
        await feed("TP2 — closing " + pos.symbol, "action", pos.addr, pos.symbol);
        const tx = pos.platform === "four_meme" ? await sellFM(pos.addr, pos.tokens) : await sellFlap(pos.addr, pos.tokens);
        if (tx) {
          await feed(pos.symbol + " closed at " + mult.toFixed(1) + "x", "confirm", pos.addr, pos.symbol);
          await logTrade(pos.addr, pos.name, pos.symbol, pos.platform, "sell", "full", pos.tokens.toString(), tx, "confirmed");
      ...
      if (mult <= C.SL) {
        ...
        if (tx) {
          ...
          await logTrade(pos.addr, pos.name, pos.symbol, pos.platform, "sell", "stop_loss", pos.tokens.toString(), tx, "confirmed");
          await closePos(pos.addr, pnl);
          positions.delete(key);
      ...
      // Time stop
      if (age >= C.TIME_STOP && !pos.halfSold) {
        await feed("time stop — " + pos.symbol, "action", pos.addr, pos.symbol);
        const tx = pos.platform === "four_meme" ? await sellFM(pos.addr, pos.tokens) : await sellFlap(pos.addr, pos.tokens);
        if (tx) {
          await feed(pos.symbol + " timed out", "system", pos.addr, pos.symbol);
          await logTrade(pos.addr, pos.name, pos.symbol, pos.platform, "sell", "time_stop", pos.tokens.toString(), tx, "confirmed");
          await closePos(pos.addr, pnl);
          positions.delete(key);
        }
      }
...
// DB
async function logTrade(a: string, n: string, s: string, p: string, side: string, bnb: string, tok: string, tx: string, st: string) {
  try { await db.from("trades").insert({ token_address: a, token_name: n, token_symbol: s, platform: p, side, amount_bnb: parseFloat(bnb) || 0, amount_tokens: tok, tx_hash: tx, status: st }); } catch {}
}

async function upsertPos(p: Pos, pnl: number) {
  try { await db.from("positions").upsert({ token_address: p.addr, token_name: p.name, token_symbol: p.symbol, platform: p.platform, entry_price_bnb: Number(p.entryPrice), amount_tokens: p.tokens.toString(), cost_bnb: parseFloat(p.cost), pnl_percent: pnl, current_multiplier: 1, status: "open", entry_time: p.time.toISOString(), updated_at: new Date().toISOString() }, { onConflict: "token_address" }); } catch {}
}

async function closePos(a: Address, pnl: number) {
  try { await db.from("positions").update({ status: "closed", pnl_percent: pnl, updated_at: new Date().toISOString() }).eq("token_address", a); } catch {}
}
...
async function heartbeat() {
  try {
    ...
    await db.from("bot_status").upsert({
      ...
      wallet_balance_bnb: parseFloat(formatEther(b)),
...
async function main() {
  ...
  console.log("  " + formatEther(bal) + " BNB\n");
