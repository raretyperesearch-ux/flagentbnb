...
var pub = createPublicClient({
  ...
  batch: { multicall: { batchSize: 200000 } },
});
  ...
  try {
    var res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 40,
        system:
          "You are Flagent, an assassin butterfly trading bot on BSC. Speak in short, calm, observational fragments. No emojis. No excitement. Under 12 words. Like a patient predator noting what it sees.",
...
async function isSafe(
  ...
  try {
    ...
    var sellTax = parseFloat(info.sell_tax || "0");
    var maxTax = Math.max(buyTax, sellTax);
    if (maxTax > 0.1)
...
async function fmInfo(token: Address) {
  try {
    ...
    var offers = r[7];
    var maxOffers = r[8];
    var liquidityAdded = r[11];
    var progress =
      maxOffers > 0n ? Number((offers * 100n) / maxOffers) : 0;
    return {
      lastPrice: lastPrice,
    ...
    var hash = await wallet.sendTransaction({
      ...
      data: encodeFunctionData({
        ...
        args: [
          {
            inputToken: token,
            outputToken: ZERO_ADDR,
            inputAmount: amount,
            minOutputAmount: 0n,
            permitData: EMPTY_BYTES,
          },
        ],
      }),
      gas: 350000n,
...
async function evaluate(
  ...
  if (!tokenName || !tokenSymbol) {
    try {
      ...
      var symbolResult = await pub.readContract({
        address: token,
        abi: ERC20_ABI,
        functionName: "symbol",
      });
  ...
  if (!sec.ok) {
    await feed(
      tokenSymbol + " rejected — " + sec.why,
      "reject",
      token,
      tokenSymbol
    );
    await think(
      "Token " + tokenSymbol + " failed: " + sec.why + ". React."
    );
    return;
  }
  ...
  await feed(
    "buying " + tokenSymbol + " — " + BUY_AMOUNT + " BNB",
    "action",
    token,
    tokenSymbol
  );
  ...
  try {
    ...
    if (receipt.status === "reverted") {
      ...
      await logTrade(
        token, tokenName, tokenSymbol, platform,
        "buy", BUY_AMOUNT, "0", tx, "failed"
      );
      return;
    }

    var bal = await pub.readContract({
      address: token,
      abi: ERC20_ABI,
    ...
    var entryPrice = 0n;
    if (platform === "four_meme") {
      var postInfo = await fmInfo(token);
    ...
    await feed(
      fmtTokens(bal) + " " + tokenSymbol + " acquired",
      "confirm",
      token,
      tokenSymbol
    );
    await logTrade(
      token, tokenName, tokenSymbol, platform,
      "buy", BUY_AMOUNT, bal.toString(), tx, "confirmed"
    );
  ...
  pub.watchContractEvent({
    ...
    onLogs: function (logs) {
      for (var i = 0; i < logs.length; i++) {
        ...
        var s = log.args.sender;
        if (!t) continue;
        if (s && s.toLowerCase() === account.address.toLowerCase()) continue;
        if (!seen.has(t.toLowerCase())) {
          evaluate(t, "four_meme");
        }
      }
    },
    onError: function (e) {
      console.error("FM watcher err:", e);
    },
  });
}
...
function startFlapScanner(): void {
  ...
  pub.watchContractEvent({
    ...
    onError: function (e) {
      console.error("Flap TokenCreated err:", e);
    },
  });
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    try {
      ...
      if (pos.platform === "four_meme") {
        var fmData = await fmInfo(pos.addr);
        if (fmData && pos.entryPrice > 0n) {
          mult = Number(fmData.lastPrice) / Number(pos.entryPrice);
        } else {
          var sellQ = await fmSellQuote(pos.addr, pos.tokens);
          if (sellQ) {
      ...
      await feed(
        pos.symbol + " " + mult.toFixed(2) + "x (" + pnlStr + ")",
        "monitor",
        pos.addr,
        pos.symbol
      );
      ...
      if (mult >= TP1 && !pos.halfSold) {
        ...
        var tp1Tx =
          pos.platform === "four_meme"
            ? await sellFM(pos.addr, half)
            : await sellFlap(pos.addr, half);
        if (tp1Tx) {
          pos.halfSold = true;
          pos.tokens = pos.tokens - half;
          await feed("secured", "confirm", pos.addr, pos.symbol);
          await logTrade(
            pos.addr, pos.name, pos.symbol, pos.platform,
            "sell", "partial", half.toString(), tp1Tx, "confirmed"
          );
          await think(
            "Sold half " + pos.symbol + " at " + mult.toFixed(1) + "x. React."
          );
        }
      }
      ...
      if (mult >= TP2 && pos.halfSold) {
        ...
        var tp2Tx =
          pos.platform === "four_meme"
            ? await sellFM(pos.addr, pos.tokens)
            : await sellFlap(pos.addr, pos.tokens);
        if (tp2Tx) {
          await feed(
            pos.symbol + " closed at " + mult.toFixed(1) + "x",
            "confirm",
            pos.addr,
            pos.symbol
          );
          await logTrade(
            pos.addr, pos.name, pos.symbol, pos.platform,
            "sell", "full", pos.tokens.toString(), tp2Tx, "confirmed"
          );
      ...
      if (mult <= SL) {
        ...
        if (slTx) {
          ...
          await logTrade(
            pos.addr, pos.name, pos.symbol, pos.platform,
            "sell", "stop_loss", pos.tokens.toString(), slTx, "confirmed"
          );
          await closePos(pos.addr, pnl);
          positions.delete(key);
      ...
      if (ageMin >= TIME_STOP && !pos.halfSold) {
        ...
        if (tsTx) {
          ...
          await logTrade(
            pos.addr, pos.name, pos.symbol, pos.platform,
            "sell", "time_stop", pos.tokens.toString(), tsTx, "confirmed"
          );
          await closePos(pos.addr, pnl);
          positions.delete(key);
        }
...
async function logTrade(
  addr: string, name: string, sym: string, platform: string,
  side: string, bnb: string, tokens: string, tx: string, status: string
): Promise<void> {
  try {
...
async function upsertPos(p: Pos, pnl: number): Promise<void> {
  try {
    await db.from("positions").upsert(
      {
        token_address: p.addr,
        token_name: p.name,
        token_symbol: p.symbol,
        platform: p.platform,
        entry_price_bnb: Number(p.entryPrice),
        amount_tokens: p.tokens.toString(),
        cost_bnb: parseFloat(p.cost),
        pnl_percent: pnl,
        current_multiplier: 1,
        status: "open",
        entry_time: p.time.toISOString(),
        updated_at: new Date().toISOString(),
      },
    ...
    await db.from("bot_status").upsert(
      {
        ...
        wallet_balance_bnb: parseFloat(formatEther(balance)),
...
async function main(): Promise<void> {
  ...
  setInterval(function () {
    feed("scanning four.meme...", "system");
  }, 60000);

  setInterval(function () {
    feed("scanning flap.sh...", "system");
  }, 75000);
